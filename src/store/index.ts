...
export default createStore({
  ...
  getters: {
    ...
    getStockFilteredAndSorted: (state) => (locationIds: Array<number>, status: string, sortParameter: string, sortUp: boolean, searchParam: string) => {
      return search(sortStockBy(filterByStatus(filterByLocation(state.stock, locationIds), status), sortParameter, sortUp), searchParam);
    },
  ...
  mutations: {
    ...
    setLocations(state, locations) {
      state.locationList.splice(1, locationList.length, ...locations);
    },
